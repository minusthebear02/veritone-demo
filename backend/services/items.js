...
async function getItemsByUser(page = 1, userId) {
  ...
  const rows = await db.query(
    `SELECT * FROM items WHERE user_id = $1 ORDER BY id ASC OFFSET $2 LIMIT $3 `,
  ...
  const meta = {
    nextPage: data.length === config.itemsPerPage ? parseInt(page) + 1 : false,
  };
...
async function createItem(item, userId) {
  ...
  let { name, description = "", purchased = false, quantity = 1 } = item;
...
async function deleteItem(itemId) {
  ...
  await db.query(`DELETE FROM items WHERE id = $1`, [itemId], (error) => {
    if (error) throw error;
  });
